...
import { Alert, AlertTitle, Avatar, ListItemIcon, ListItemText, MenuList, Typography } from '@mui/material';
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <Paper sx={{ py: "10px", width: '100%' }}>
          <RubberBand spy={converterType}>
          <MenuList>
            {
              converters_type.map((obj) => (
                <>
                  <MenuItem key={obj.id} onClick={() => {changeConverterType(obj.value)}}>
                    <ListItemIcon>
                      <Avatar sx={obj.value==converterType.toString()?{bgcolor: green[500]}:{bgcolor: blue[500]}}variant="rounded">
                        {obj.icon}
                      </Avatar>
                    </ListItemIcon>
                    ...
                    {
                      obj.value==converterType.toString()?(<CheckCircleIcon color="success" />):(<CheckCircleIcon color="disabled" />)
                    }
                  </MenuItem>
                </>
              ))
